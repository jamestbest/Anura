...
== Failed disassembly
ISDL does not currently implement all the x64 instructions and therefore can find unknown instruction opcodes. In this case there must be some recovery mechanism to make sure that data is not decoded as instructions, and that Anura can get back to decoding instructions correctly. With DWARF information there is a mapping of the source line numbers to their instruction address. This means there are known locations that are the start of instructions. 

#figure(
  caption: [Example line-address mappings],
table(
  columns: (auto, 1fr, 1fr),
...
With these mappings the debugger can disassemble until it reaches an unknown operation, at which point it can find the next line in the program and get its start address. The area between the current address and the next start address must be filled with a representation of 'unknown bytes', in Anura's case this is '???'. Disassembly can then continue from the start address. Anura also uses the virtual subroutine information when disassembling, iterating through and applying the disassembly recovery if needed.

@DissSkip shows the output of Anura when disassembling a subroutine with two instructions that have not been implemented in ISDL, namely the sete and movzx instructions, in this case even if the movzx instruction had been implemented it would not have been decoded because it is not the start of a source line, leave and ret are displayed as they are associated with the last line of the subroutine.
